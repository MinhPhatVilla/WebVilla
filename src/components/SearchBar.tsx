...
export default function SearchBar({ onSearch, isCompact = false }: SearchBarProps) {
    ...
    return (
        <div ref={searchRef} className="relative w-full max-w-4xl mx-auto">
            ...
            <button
                className="md:hidden flex items-center justify-between w-full bg-white rounded-full shadow-md border border-gray-100 p-3 px-5 transition-transform active:scale-95"
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-xs text-gray-500 flex gap-1 items-center">
                            <span>{checkIn && checkOut ? `${formatDate(checkIn)} - ${formatDate(checkOut)}` : "Thời gian"}</span>
                            <span>•</span>
                            <span>{guests ? `${guests} khách` : "Thêm khách"}</span>
                        </p>
                    </div>
                </div>
            ...
            {/* MAIN SEARCH PANEL (FULLSCREEN ON MOBILE, INLINE ON DESKTOP) */}
            <div className={`
                ${isMobileModalOpen ? "fixed inset-0 z-[100] bg-gray-50 flex flex-col h-[100dvh] transition-all duration-300 animate-in slide-in-from-bottom" : "hidden md:block"}
            `}>
                ...
                {isMobileModalOpen && (
                    <div className="bg-white px-4 py-4 flex items-center justify-between shadow-sm sticky top-0 z-50">
                        <button
                            ...
                            className="w-10 h-10 flex items-center justify-center bg-gray-100 hover:bg-gray-200 rounded-full text-gray-900 transition-colors"
                        >
                            <X size={20} />
                        </button>
                        <span className="font-bold text-gray-900 text-lg">Tìm kiếm</span>
                        <div className="w-10 flex justify-end">
                            <button onClick={() => { setLocation(""); setCheckIn(""); setCheckOut(""); setGuests(1); setActiveField(null); }} className="text-xs font-bold text-gray-500 underline uppercase tracking-wider">Xóa</button>
                        </div>
                    </div>
                )}

                <div className={`
                    flex flex-col md:flex-row md:items-center bg-white md:rounded-full md:shadow-lg border-2 border-transparent md:border-2 transition-all duration-300
                    ${!isMobileModalOpen && activeField ? "md:border-cyan-400 md:shadow-2xl md:shadow-cyan-100" : "md:border-gray-100 md:hover:shadow-xl"}
                    ${isMobileModalOpen ? "rounded-3xl mx-4 mt-6 shadow-xl border-gray-100" : ""}
                `}>
                ...
                {/* ========== DROPDOWNS ========== */}
                {activeField && !isMobileModalOpen && (
                    <>
                        {/* Desktop: render BOTH overlay + dropdown via Portal to escape stacking context */}
                        {typeof document !== 'undefined' && createPortal(
                            <>
                                ...
                                />
                                {/* Dropdown Container */}
                                <div ref={portalRef} className="fixed left-1/2 -translate-x-1/2 top-40 flex justify-center"
                                    style={{ zIndex: 9999 }}
                                >
                                    {/* Location Dropdown */}
                                    {activeField === "location" && (
                                        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-6 w-80 animate-in slide-in-from-top-2 max-h-[85vh] overflow-y-auto">
                                            <div className="flex items-center justify-between mb-4">
                                    ...
                                    {/* Check-in Calendar */}
                                    {activeField === "checkin" && (
                                        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-6 w-[340px] mx-auto animate-in slide-in-from-top-2 max-h-[85vh] overflow-y-auto">
                                    ...
                                    {/* Guests Counter */}
                                    {activeField === "guests" && (
                                        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-6 w-80 ml-auto animate-in slide-in-from-top-2 max-h-[85vh] overflow-y-auto">
                ...
                {/* Mobile dropdowns (inside component tree, not portal) */}
                {activeField && isMobileModalOpen && (
                    <div ref={portalRef} className="relative flex-1 overflow-y-auto px-4 pb-28"
                        style={{ zIndex: 9999 }}
                    >
                        {/* Location Dropdown */}
                        {activeField === "location" && (
                            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 w-full animate-in slide-in-from-bottom-full max-h-[85vh] overflow-y-auto mt-4">
                                <div className="flex items-center justify-between mb-4">
                                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Chọn khu vực</p>
                                    <button onClick={(e) => { e.stopPropagation(); setActiveField(null); }} className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full text-gray-500 hover:bg-gray-200">
                                        <X size={16} />
                                    </button>
                                </div>
                                <div className="space-y-2">
                ...
                {isMobileModalOpen && (
                    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 pb-6 z-[110] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                        <button
                            onClick={handleSearch}
                            className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-rose-500 to-pink-600 text-white p-4 rounded-full hover:from-rose-600 hover:to-pink-700 transition-all shadow-lg active:scale-95"
                        >
                            <Search size={20} />
                            <span className="font-bold text-lg">Tìm kiếm {nights > 0 ? `(${nights} đêm)` : ""}</span>
                        </button>
                    </div>
                )}
            </div>
